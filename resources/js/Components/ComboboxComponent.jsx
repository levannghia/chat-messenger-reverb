...
export default function ComboboxComponent({ url, onChange: updateData, initialSelected, disabled, refId }) {
    ...
    useEffect(() => {
        ...
        window.axios.get(`${url}?query=${query}`)
            .then((response) => {
                setPeople(response.data.data.filter((person) =>
                    selectedPerson.find((selected) => selected.id === person.id) === undefined,
                ))
            })
    }, [debouncedQuery])
    ...
    const removeSelectedPerson = (value) => {
        const filterdPerson = selectedPerson.filter((selected) => selected.id !== value.id)
        setSelectedPerson(filterdPerson);
        updateData(filterdPerson.map((val) => val.id))
    }
                ...
                {selectedPerson.length > 0 && (
                    <ul className="m-2 flex flex-wrap gap-1 text-xs">
                        {selectedPerson.map((person) => (
                            <li key={person.id} className='flex items-center justify-between gap-2 rounded-full bg-primary px-2 py-1 text-white'>
                            ...
                            >
                                {({ active, selected }) => (
                                    <>
                                        ...
                                        {selected && <span className="absolute inset-y-0 right-2 flex items-center pl-3 text-foreground">
